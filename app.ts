...
async function main(): Promise<void> {
  ...
  const streamIdStorePath = resolve(__dirname, 'src/stream-id-store.json');
  const demoConfigPath = resolve(__dirname, 'src/demo-config.json');
  
  let streamIdStore: any, demoConfig: any;
  try {
    streamIdStore = JSON.parse(await readFile(streamIdStorePath, 'utf-8'));
    demoConfig = JSON.parse(await readFile(demoConfigPath, 'utf-8'));
  } catch (err) {
  ...
  try {
    switch (command) {
      ...
      default:
        ...
        if (!isNaN(stepNumber)) {
          ...
          const updatedStreamIdStore = JSON.parse(await readFile(streamIdStorePath, 'utf-8'));
    ...
    console.error('Error in app.js:', error instanceof Error ? error.message : String(error));
    process.exit(1);
  }
}
